...
function dehyphen(sym::Symbol)
    name = string(sym)
    if '-' in name
        @warn "Dehypen" name
        Symbol(replace(string(sym), '-'=>'_'))
    else
        sym
    end
end
...
function lift_lowered_expr!(ex, nextids, valmap, lift_full)
    ...
    if ex isa Symbol
        ...
        # Replace `-` with `_` in lisp special forms.  Should now be converted
        # in lisp code.
        return dehyphen(ex)
    end
    if ex isa Expr
        ...
        if lift_full
            ...
            elseif ex.head in [:_while, :_do_while, :scope_block, :break_block,
                ...
                return Expr(:macrocall, Symbol("@Ex"), nothing,
                            QuoteNode(dehyphen(ex.head)), ex.args...)
                            =#
            end
        end
...
function lower_ref_expr!(ex)
    if ex isa Expr
        ...
        elseif ex.head == :let && isempty(ex.args[1].args) # FIXME!
            return Expr(:scope_block, ex.args[2])
        elseif ex.head == :macrocall && ex.args[1] == Symbol("@Ex")
            head = ex.args[3]
...
@testset "Loops" begin
    ...
    @test_desugar(
        ...
        @Ex(:break_block, loop_exit,
            @Ex(:_while, cond,
                @Ex(:break_block, loop_cont,
                    @Ex(:scope_block, begin
                            body1
                            @Ex :break loop_cont
                            body2
                            @Ex :break loop_exit
